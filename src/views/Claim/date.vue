<template>
    <div>
      <v-card class="mx-6 ma-10 pt-4" color="#f0f0f0cc" height="300px">
        <template>
          <v-container fluid>
            <v-row align="center">
            ...
            <div class="d-flex justify-center">
                <v-btn color="ma-2 px-12 teal white--text" >
                    save
                </v-btn>
            </div>
          </v-container>
  ...
  export default {
    ...
    methods: {
      ...mapActions(["addClaimAction","set_attr_CLAiMAction"]),
      initialize() {
        if (this.geteditedOrSavedclaim.id > 0) {
          this.dateClaim.incident_date =
            this.geteditedOrSavedclaim.incident_date;
          this.dateClaim.claim_date =
            this.geteditedOrSavedclaim.claim_date;
      ...
      saveDate() {
        this.addClaimAction(this.dateClaim);
