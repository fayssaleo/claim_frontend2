    ...
    SET_CLAiMS(state, claims) {
      for (let index = 0; index < claims.length; index++) {
        ...
        if (claims[index].nature_of_damage == null) {
          claims[inedx].nature_of_damage = { id: 0, name: "" };
        }
        if (claims[index].type_of_equipment == null) {
          claims[inedx].type_of_equipment = { id: 0, name: "" };
        }
      }
      state.claims = claims;
    },
  ...
  actions: {
    ...
    editedOrSavedEquipmentClaimAction({ commit }, claim) {
      return new Promise((resolve, reject) => {
        ...
        claimFormData.append("incident_date", claim.incident_date);
        claimFormData.append("claim_date", claim.claim_date);
